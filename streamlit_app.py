...
UX:
- Glassmorphism dark-themed form (selectbox, text_input, slider)
...
- Telemetry expander for latency / model info
"""
...
<style>
    ...
    .rec-insight strong { color: #a78bfa; }
    .ai-badge {
        display: inline-flex;
        align-items: center;
        gap: 6px;
    ...
    .chip-label { color: #6366f1; font-weight: 600; }
    .empty-icon { font-size: 2.5rem; text-align: center; display: block; margin-bottom: 0.5rem; }
    .empty-headline { color: #94a3b8; font-size: 1.1rem; font-weight: 600; text-align: center; }
    .empty-subtext { color: #64748b; text-align: center; font-size: 0.9rem; }
...
with st.form("preferences_form"):
    ...
    rating = st.slider(
        ...
        value=3.5,
        step=0.5,
        help="Drag to set the minimum acceptable rating",
    )
    ...
    with st.spinner("🍽️ Analyzing restaurants with AI..."):
        try:
            ...
            if prefs.get("location"):
                chips_html += f'<span class="chip"><span class="chip-label">Loc:</span> {prefs["location"]}</span>'
            if prefs.get("budget"):
                chips_html += f'<span class="chip"><span class="chip-label">Budget:</span> {prefs["budget"].capitalize()}</span>'
            if prefs.get("cuisine"):
                chips_html += f'<span class="chip"><span class="chip-label">Cuisine:</span> {prefs["cuisine"]}</span>'
            if prefs.get("min_rating", 0) > 0:
                chips_html += f'<span class="chip"><span class="chip-label">Rating:</span> {prefs["min_rating"]}+</span>'
            if chips_html:
                st.markdown(f'<div style="margin-bottom: 0.75rem;">{chips_html}</div>', unsafe_allow_html=True)

            # ── AI BADGE ──
            is_ai = result.source == "llm"
            if is_ai:
                model_str = result.llm_model or "unknown"
                badge_html = f'<div class="ai-badge green">✨ Powered by {result.llm_provider} ({model_str}) — {result.latency_ms or elapsed_ms}ms</div>'
            else:
                badge_html = '<div class="ai-badge amber">⚙ Rule-based Ranking (AI Unavailable)</div>'
                ...
                st.json(
                    {
                        ...
                        "total_candidates": result.filter_result.total_matches if result.filter_result else 0,
                        "relaxed_filters": result.filter_result.relaxed_filters if result.filter_result else [],
